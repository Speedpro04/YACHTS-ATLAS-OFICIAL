...
28. **Consentimento do titular para compartilhar** — a avaliação de 30/08 dizia que "não existe base legal registrada". **Estava errada, e a favor do sistema**: o fluxo de compartilhamento existe inteiro em `api/v1/dossie.py`. O terceiro solicita declarando quem é e **para qual finalidade** (`dossie_solicitacoes`), alguém libera (fica gravado `liberado_por` e `liberado_em`), o acesso exige senha e conta `acessos`/`ultimo_acesso`, e a entrega vira linha em `dossie_saidas` com destinatário, e-mail, finalidade, canal, marina e ativo. Isso é um **livro-razão de saídas** e responde a pergunta difícil da LGPD: "quem recebeu, quando e para quê". As duas tabelas estão zeradas só porque ninguém pediu dossiê ainda. **O que falta de fato é menor:** o registro do CONSENTIMENTO do armador — hoje quem libera é a plataforma, e não há linha do dono dizendo "pode compartilhar o histórico do meu barco".
28b. **O registro da saída é `best-effort` e pode não acontecer** — em `dossie.py:524` o insert em `dossie_saidas` está dentro de `try/except: pass`. Se falhar, **a entrega acontece e o rastro não**. Num livro-razão de compartilhamento isso é o inverso do desejado: melhor recusar a entrega do que entregar sem registrar. E `dossie_saidas` não tem gatilho de imutabilidade, diferente de `dossie_emitidos`.
