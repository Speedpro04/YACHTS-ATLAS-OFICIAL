- [x] **REV-21 — O vídeo entra na home, e o lugar foi medido, não escolhido no gosto**: pedido do fundador (*"ache o melhor local na página home"*). Ficou **logo depois do hero, antes de "O Dossiê Atlas"**. A medição na página rodando: o CTA *"Quero na minha Marina"* fecha em **807px**, o hero vai até **947px** (o `pb-[140px]` reservado para o indicador "ROLAR"), e a página inteira tem **12.118px**. Ou seja: o vídeo é a **primeira coisa que aparece na primeira rolagem**. O próximo lugar decente seria a 2.531px — quase três telas — e vídeo que exige rolar não é visto, ainda mais o que existe para dar clareza a quem desistiria primeiro. **Correção de algo que eu disse errado ao fundador:** afirmei que o pôster ia "espiar 93px acima da dobra"; não vai. Aqueles 93px são do `pb-[140px]` do hero, que é espaço reservado e documentado do REV-17 — mexer nele para o vídeo aparecer seria trocar uma decisão de layout já tomada por outra sem pedir.
- [x] **REV-21 — Remendar o vídeo pronto não funcionava; remontamos dos 4K originais**: o vídeo do Lançamento tem a **legenda queimada nos pixels** (as duas versões, inclusive a `v1-audio-antigo`, cuja legenda ainda diz "18 meses" — que não pode aparecer na Oficial). Trocar só o áudio deixaria a pessoa **lendo uma frase e ouvindo outra**. Três tentativas de esconder falharam na frente do fundador: **borrão** (deixava o fantasma do texto legível), **tarja opaca** (*"essa tarja preta está horrível"* — flutuava no meio do quadro e cortava o barco ao meio) e **degradê até a borda** (*"essa faixa preta está horrível"*). E cortar a faixa fora custava 305 das 1080 linhas — *"a parte inferior do vídeo está cortada"*. A saída apareceu ao ler o repositório `video-yacht-atlas`: a montagem **está em código** (`montar_final.py`), e os **6 clipes 4K de origem existem** em `C:\01-heygen-github\videos\`. Refeito do zero com `remontar.py`: mesma sequência de 16 trechos, mesma abertura e fecho de logo, `factor` recalculado para o áudio novo de 2:58. **A imagem nasce limpa — nunca teve legenda para esconder** — e a legenda nova entra com **caixa só atrás das palavras** (`BorderStyle=3`), que foi o pedido. Master 230,7 MB; entregue a 1080p/3,5 Mbps em **75,3 MB**.
- [x] **REV-21 — A legenda saiu de transcrição de verdade, depois de dois chutes errados**: alinhar texto conhecido a áudio parece fácil e não é. **Tentativa 1**, distribuir os caracteres proporcionalmente ao tempo de fala: saiu **2 a 4 segundos atrasada**. **Tentativa 2**, ancorar as quebras nas N-1 maiores pausas do `silencedetect`: saiu **16 segundos adiantada** — pior, porque pausa grande não cai só em fim de frase. Legenda 16s fora é pior que legenda nenhuma, então instalei o `faster-whisper` e transcrevi o áudio com **tempo por palavra** (452 palavras). O texto do fundador foi casado com a transcrição por **alinhamento global** (`SequenceMatcher`, não guloso — o guloso desandava no fim e jogava a última fala 25s para trás): **96% das palavras ancoradas**. Confere: *"apaga"* em 1:54,4 e *"chamar"* em 2:57,8, ambos batendo com o áudio. Sobrou registrado que a **grafia diverge de propósito** — o áudio fala "Iate Atlas" e "S-H-A 256", a legenda escreve **Yachts Atlas** e **SHA-256**: o ouvido aprende a pronúncia, o olho aprende a grafia.
- [x] **REV-21 — Responsivo conferido na régua, não no olho**: o pedido foi *"responsivo sempre"*. Medido com a página rodando, em 1920 e em 375: moldura **898×505** no desktop e **336×189** no celular, **proporção 1.778 nos dois** (16:9 exato, que é o formato do arquivo). **Zero rolagem horizontal** a 375px. E o que mais importa no celular: **o `.mp4` de 75 MB não é baixado no carregamento** — confirmado por `performance.getEntriesByType('resource')`, que não lista o arquivo até o clique. Sem isso, todo visitante de 4G pagaria 75 MB para ver uma página que talvez nem role. A moldura ganhou **raio de 3px e linha dourada de 1,5px**, a mesma escrita da moldura do Dossiê (`border-[1.5px] border-[#c5a059] rounded-[3px]`) — o fundador pediu 2px, viu, e voltou para 1,5. Vale lembrar o que o REV-19 já documentou: **1,5px computa 1px em tela DPR 1** porque o Chrome arredonda para pixel de dispositivo; no Retina sai cheia. Não é defeito e não se "conserta".
- [x] **REV-21 — Os 75 MB entram no histórico do git, e isso é para sempre**: o `video-programa-atlas.mp4` foi para `frontend/public/`, que é o padrão que o Lançamento já usa (`/video-yacht-atlas.mp4` servido da raiz). O custo está dito por escrito porque não dá para desfazer sem reescrever o repositório: hoje o maior arquivo versionado da Oficial tem **12 MB**, e o `.git` do Lançamento já está em **167 MB** justamente por causa de vídeo. Cada versão nova do vídeo soma outro tanto. A alternativa oferecida — servir de fora, do repositório `video-yacht-atlas`, deixando aqui só a URL — **foi descartada pelo fundador**, que mandou commitar assim. Se um dia isso apertar, o remédio é `git-filter-repo` ou LFS, e os dois exigem reescrever histórico.
- [x] **REV-20 — A reserva de 3 horas não cabia num conselho de Marina**: pedido do fundador (*"uma hora é muito pouco"*, depois *"vamos aumentar para 12h"*). O prazo virou **12 horas** (`MINUTOS_DE_RESERVA_VAGA` 180 → **720**). O motivo é de venda, não de código: **quem preenche o cadastro raramente é quem decide** — o gerente precisa levar os US$ 200/mês ao conselho ou ao dono da Marina, e conselho que só se reúne à tarde não cabe em três horas. Meio dia cobre o expediente inteiro sem prender a vaga de um dia para o outro. **O número morava em três lugares e só um era o certo**: os **60 minutos** do `reservar_vaga_fundadora` (padrão da função no banco, que o backend sempre sobrescreve), os **180** do `config.py` (o valor real) e o **"3 horas"** escrito à mão no `RegistroMarina.tsx`, que era o único que a Marina lia. Mudar só o backend faria a página mentir. Os três andaram juntos, e cada ponta ganhou comentário dizendo onde mora o outro. Sobras varridas no mesmo passe: `leads.py:116`, `stripe_service.py:699` e o `PRD.md` do Lançamento. As duas menções em `stripe_service.py:670` e `PRD.md` (linha 41 e 869) **ficaram em 3h de propósito** — são o relato do incidente de 27/08, quando o prazo era esse; corrigir número de história é falsificá-la.
- [x] **REV-20 — O texto do prazo virou argumento de venda, nas duas páginas**: o fundador pediu psicologia no aviso (*"NOVO, DIGITAL, BANCO DE DADOS"*), e o exemplo que ele deu — *"sua Marina tem 12 horas para entrar no Programa Atlas"* — prometia porta fechada, que é falso: o prazo solta a vaga, não encerra o Programa, e a Marina pode reservar de novo (o `reservar_vaga_fundadora` **renova a mesma linha**, não duplica). Vender imutabilidade com relógio falso é o pior lugar possível para esticar a verdade. Ficou o que já era verdade e pesa mais: **são 4 no estado dela e a vaga volta para a fila**. Texto final nas duas páginas, idêntico: *"Sua vaga está reservada por 12 horas — tempo de levar a decisão a quem precisa aprovar. Ao concluir, tudo o que a sua Marina faz passa a existir em banco de dados: cada serviço com data, autor e assinatura digital, no nome dela. É o primeiro dia em que o trabalho dela deixa de depender de quem lembra. Passadas as 12 horas, a vaga volta para as 4 do seu estado."* Entrou em `RegistroMarina.tsx` (Oficial) e no rodapé do modal de cadastro do Lançamento (`index.html`), que vive em **repositório separado** — o próprio arquivo já registra que os avisos das duas páginas são iguais **de propósito**, pela mesma razão da regra de senha que morou em três lugares com três valores.
- [x] **REV-20 — As 20 vagas estavam em 19, e ninguém tinha visto**: ao conferir o que segurava vaga em produção, São Paulo aparecia com **3 restantes de 4**. A vaga faltante era da **Antioquia Marina** (`activescertus@gmail.com`), o teste de pagamento de **27/08** — status `ativo`, que **não vence nunca**. No dia do lançamento SP abriria anunciando 3 vagas, e uma das 20 sumiria sem explicação. **Arquivada, não apagada**: a tabela já tem `cancelado` no `CHECK`, e a contagem só olha `ativo` ou reserva dentro do prazo — trocar o status libera a vaga **e mantém o `cs_live_...` guardado na linha**, que é o único rastro se um dia pintar cobrança estranha. Apagar levaria a evidência junto com a vaga. Junto foram as duas reservas de teste de 19 e 20/08 (`kd3online@`, `axushub50@`), já vencidas e sem Stripe. **20 de 20, 4 em cada estado.** A tabela antiga do piloto de 3 vagas (`founder_program_spots`) foi conferida no mesmo passe: as 5 linhas estão `available`, não seguram nada.
- [x] **REV-20 — Cinco testes de vaga, rodados em transação desfeita**: testar reserva em produção **gasta vaga de verdade** — reservar 4 em SC para ver o limite deixa SC cheio, com Marina falsa dentro. Os testes rodaram num bloco `DO` que termina em `RAISE EXCEPTION` de propósito: o Postgres desfaz tudo e o relatório volta na mensagem do erro. Conferido depois: **0 linhas de teste sobraram**. Passaram: o **cap de 4 por estado** (a 5ª caiu em `vagas_do_estado_esgotadas` e foi para a oferta Oficial de $250), **estado fora do programa** (MG → `tradicional`, $250), **reserva repetida** (renova, não duplica), e **pagamento atrasado honrado** (reserva vencida derrubou ES para 3, o pagamento tardio devolveu para 4). O item da **expiração** não precisou de teste: a produção já tinha provado sozinha, com as duas reservas de agosto que venceram e soltaram a vaga sem ninguém mexer. **Correção de um alarme falso meu**: o quinto teste fez ES terminar com **5 ocupadas**, e eu levei isso ao fundador como risco de virar 21 fundadoras. Ele chamava `cadastrar_marina_fundadora` **direto, pulando o webhook** — e é o webhook que barra: `stripe_service.py:699` só honra reserva vencida **se o estado ainda tiver espaço**, e grita quando não tem. Pelo caminho real não passa.
- [x] **REV-20 — O prazo curto não protege de pagamento atrasado, e não dá para consertar por código**: o checkout do Lançamento é **payment link da Stripe** (`buy.stripe.com`, `config.py:229`), não sessão criada por nós — então `expires_at` está fora do nosso alcance e o link vive **24h**, independente do que a reserva diga. Com 12h de reserva, sobram 12 horas em que a Marina consegue pagar uma vaga que já voltou para a fila. **Quem cobre isso não é o relógio, é o aviso**: `_avisar_pagamento_sem_vaga` manda a decisão para o humano — honrar como fundadora, migrar para $250 ou reembolsar. **Pendência fora do alcance daqui**: `MINUTOS_DE_RESERVA_VAGA` é campo de `Settings` do pydantic, então uma variável de ambiente com esse nome no servidor **ganha do código** e as 12h não valem em produção. É a mesma pegadinha dos 5min/60s da prospecção, que estão em produção diferentes do padrão do código. Precisa ser conferido no ambiente do deploy.
- [x] **REV-19 — A senha vazada sem plano Pro, e o buraco que o Pro nem cobriria**: o fundador foi ligar a proteção contra senha vazada e descobriu que **só existe no plano Pro**. Ao procurar a alternativa, apareceu coisa pior: as quatro regras (10 caracteres, maiúscula, minúscula, número) viviam **só no `RegistroMarina.tsx`**, conferidas no navegador. No backend, `MarinaRegistroPublico.password` era `str` puro e ia **direto para o `create_user` do Supabase** — quem postasse em `/leads/marina/registrar` cadastrava com `"123"`, e o único piso real era o mínimo do próprio Supabase, que vem em **6**. Pior: `UsuarioCreate` dizia `min_length=8` — **três números para o mesmo fato** (site 10, schema 8, Supabase 6). Agora a regra mora em `app/core/security.py` (`SENHA_MINIMO`, `_REGRAS_SENHA`, `faltas_na_senha`, `validar_senha`) e os **dois** modelos a chamam por `field_validator`. O `SENHA_MINIMO` do frontend ganhou comentário dizendo que é **espelho** e que a autoridade é o backend.
- [x] **REV-19 — A checagem de vazamento, feita por nós, de graça**: `senha_vazada()` consulta o **mesmo HaveIBeenPwned** que o recurso pago do Supabase usa, pela API de faixa, que é **pública e não pede chave**. Funciona por **k-anonimato**: manda só os **5 primeiros caracteres** do SHA-1 e compara os sufixos localmente — a senha nunca sai do servidor e o hash completo também não. Vai com `Add-Padding: true`, senão o **tamanho** da resposta já entrega quantos hashes existem naquela faixa para quem observa a rede. **Falha ABERTO de propósito**: HIBP fora do ar devolve `False` e registra no log — travar cadastro de Marina pagante por serviço de terceiro seria pior que o problema. A prova de que isso valia: **`"Password123"` passa nas quatro regras** (11 caracteres, maiúscula, minúscula, número) **e está vazada** — testado ao vivo contra o HIBP. **16 testes novos** em `test_politica_de_senha.py`, incluindo um que prova que a URL chamada contém os 5 primeiros do hash e **não contém** a senha, o hash completo nem o sufixo. Suíte: 392 → **408 passando**.
- [x] **REV-19 — Passe nos advisors do Supabase: 21 políticas de RLS liam `auth.uid()` uma vez por LINHA**: pedido do fundador (*"ajuste tudo"*). O advisor de performance do projeto BR (`owzelkiyorumnlaycral`) tinha **104 avisos**. O de maior peso era `auth_rls_initplan`: 21 políticas chamavam `auth.uid()`/`auth.role()` dentro do `USING`, e o Postgres reavaliava a função **a cada linha lida**. Envolver em `(select auth.uid())` faz virar InitPlan — uma avaliação por consulta. **A lógica de quem vê o que não muda em nenhuma das 21**; conferido depois: políticas ainda por linha = 0 e total de políticas = **34, o mesmo de antes** (nenhuma criada nem removida). Junto, **10 chaves estrangeiras sem índice de cobertura** ganharam índice — sem ele, todo DELETE/UPDATE no lado pai varre a filha inteira. A primeira tentativa da migration **falhou e não aplicou nada** (é transacional): eu deduzi o nome da coluna pelo nome da FK e `registros_lgpd_solicitacao_fkey` aponta para `lgpd_solicitacao`, **sem o sufixo `_id`**. Resultado: `auth_rls_initplan` 21 → **0**, `unindexed_foreign_keys` 10 → **0**, total 104 → 83. SQL arquivado em `migration_rls_initplan_e_indices_fk.sql`.
- [x] **REV-19 — O que os advisors apontam e NÃO é defeito, com o motivo**: os **14 avisos de "RLS ligado sem política"** são as tabelas *deny-all* de backend — o comentário de cada uma já diz que a ausência é deliberada e é a configuração mais restritiva possível. O linter não lê comentário. Os **20 `multiple_permissive_policies`** são só **3 pares** de verdade: `audit_logs` SELECT (usuário + admin), `insurance_integrations` SELECT (dono + seguradora) e `founder_program_spots` ALL — este último com as duas políticas em `using(false)` para papéis diferentes (`anon` e `authenticated`), ou seja, deny-all explícito. **Deixados de propósito:** fundir "Usuários veem seus próprios logs" com "Admins veem todos os logs" numa política só ganharia nada mensurável a 188 linhas e **custaria a leitura de auditoria** — duas políticas nomeadas dizem a intenção; uma fundida esconde. Os **63 `unused_index`** (53 + os 10 novos, ainda sem uso porque acabaram de nascer) também ficam: *não usado* significa *não usado desde o último reset de estatísticas*, e `idx_audit_logs_severity` passa a ser usado no primeiro filtro por severidade. A qualquer escala abaixo de alguns milhares de linhas, esses avisos são teoria.
- [x] **REV-19 — Dois achados de infraestrutura**: (1) **só 2 dos 4 bancos são alcançáveis** pela conexão MCP atual — BR (`owzelkiyorumnlaycral`) e US (`zanroywfeufpkdlbfhxo`). Europa (`pjwkfqrxhyhlntxwjqqp`) e LATAM (`rxnheiegidrlmuvewtdk`) existem, mas em **outra organização Supabase**; não dá para auditar nem ajustar daqui. (2) **O banco do US já estava limpo**: zero políticas com o problema de InitPlan e zero FKs sem índice — nada a fazer lá. Pendente e fora do alcance do MCP: **proteção contra senha vazada está DESLIGADA nos dois projetos** — é chave de painel, um clique, e impede cadastro com senha já vazada. E a extensão `vector` está no schema `public` nos dois; mover mexe no pgvector que sustenta o RAG **em produção**, então fica para janela calma.
- [x] **REV-19 — A seção do Dossiê Atlas: tipografia, moldura e a palavra que faltava**: pedido do fundador olhando a tela. A headline caiu de **48px para 44px** (`text-4xl md:text-5xl` → `text-[2rem] md:text-[2.75rem]`), o que resolveu a quebra que deixava "emite" sozinho na segunda linha — medido: o texto tem **1.129px** de largura natural a 44px, então cabe em uma linha **de 1.177px de viewport para cima**; abaixo disso quebra, e deve quebrar mesmo. O subtítulo entrou numa **moldura de 1,5px dourada com raio 3px**, fundo transparente (o `#010c20` da seção atravessa), 32px/40px de respiro e `max-w-2xl` — a largura subiu de `xl` para `2xl` de propósito, para o texto manter a mesma medida útil depois do padding. **Pegadinha do 1,5px**: o Chrome computa `borderTopWidth: 1px` em tela DPR 1 porque arredonda para pixel inteiro de dispositivo — o CSS está certo, confirmado com elemento de prova (`border:1.5px` inline computa 1px igual). Em Retina sai cheio; **não "consertar" isso**. E o fundador viu o que a moldura expôs: a frase emoldurada **não falava de venda**. Ao conferir as três línguas apareceu o motivo — `en.json` (*"Trust closes deals faster"*) e `es.json` (*"La confianza acelera cierres"*) **falam**, só o `pt.json` não. Quando a Oficial foi reposicionada para B2B em 30/08, **só o português foi reescrito**; EN e ES ficaram com a copy antiga de armador, e o argumento de venda sobreviveu neles por acidente. O PT passou a dizer "...em **prova na hora da venda** — o dono vê, o corretor aceita e a seguradora reconhece". **EN e ES ficaram como estavam por decisão dele** ("deixa os outros por enquanto") — segue pendente que as duas línguas inteiras ainda vendem para o armador.
...
Fica junto do que já estava ali (o vídeo e as 12 horas de reserva — eram 3 quando isto foi escrito, ver REV-20), no mesmo bloco — não em outro lugar da tela.
...
Isso decide o caso da **reserva vencida**. As 12 horas (3 quando isto foi escrito, ver REV-20) existem para ninguém parar em cima de uma vaga sem pagar; passado o prazo, ela volta para a fila (`fn_vagas_fundadoras_ocupadas` só conta reserva dentro do prazo) — **mas a linha continua no banco**. Honrar essa linha sem olhar o estado criaria a quinta fundadora de SP.
