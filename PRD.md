...
27. **CPF/CNPJ do proprietário em texto plano** — 7 dos 14 ativos têm `proprietario_documento`, e os 7 estão em claro. `pgcrypto` está instalado (schema `extensions`) e não é usado. O dossiê já mascara na exibição (`_mascarar_documento`), mas no banco está aberto. É o item que reprova numa auditoria LGPD, e é pré-requisito para conversar com seguradora.
28. **Base legal para compartilhar não existe** — há `lgpd_solicitacoes` (direito do titular: acesso, eliminação), mas nenhuma tabela de **consentimento**. Quando o dossiê for por API para uma seguradora, a pergunta é com que autorização do armador, e ela precisa estar datada e registrada.
...
30. **13 tabelas com RLS sem política** — é deny-all e está seguro (só o backend, com service_role, alcança). Mas um auditor vai perguntar, e "é intencional" precisa estar escrito, não explicado de viva voz.
31. **Proteção contra senha vazada desligada** — Supabase → Authentication → Policies → ativar HaveIBeenPwned. Controle esperado em ISO 27001, 1 minuto, depende do fundador (o MCP não expõe configuração de Auth).
32. **Landing carrega o app inteiro** — o build é um arquivo único de **852 KB** e `App.tsx` não usa `React.lazy` em nenhuma rota. Quem abre a home baixa o painel da marina, o `@supabase/supabase-js` e o `lucide-react` completos antes de ver a primeira letra — nenhum dos três é usado na landing. Foi o que o fundador sentiu como "página um pouco lenta" em 30/08/2026. Conserto é code splitting por rota; mexe em arquitetura, então aguarda autorização.
