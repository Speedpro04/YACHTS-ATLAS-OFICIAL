...
40. **Os 14 `rls_enabled_no_policy` do linter precisam de explicação escrita** — não são defeito: são o padrão *deny-all* adotado de propósito (RLS ligada sem política = nada alcança pela API; o backend usa a chave de serviço). Mas o linter marca em INFO porque a mesma configuração apareceria se alguém tivesse **esquecido** de criar a política — ele não distingue intenção de esquecimento. Um auditor lendo a saída crua vai perguntar. Escrever isso no dossiê de conformidade **antes** de alguém perguntar, não depois.
