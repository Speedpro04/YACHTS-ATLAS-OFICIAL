...
16. **Opt-out da prospecção não funciona sozinho (BLOQUEIA O PRIMEIRO DISPARO)** — a mensagem promete "responda SAIR", mas não há webhook da Evolution ligado: hoje só sai da lista quem for bloqueado à mão (`prospeccao_service.bloquear`). Prometer saída e não entregar é o que gera denúncia — e denúncia é o que bane o número.
